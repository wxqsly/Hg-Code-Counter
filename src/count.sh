...
then
	...
	do
			...
			user_log="$report_dir/count_user_$i.log"
			#user_log=count_user_$i.log
			
			#Ignore the merged revisions with -M para
			hg log --stat -d "$count_from to $count_end" -u "${user}" --template "{files}" -M > "$user_log"
			...
			added=`cat $user_log |grep 'files changed'| awk -F ',' '{print $2}'|awk '{sum +=$1} END {print sum}'`
			if [[ ! $added ]];
				then 
				added=0
			fi
			...
			deleted=`cat $user_log |grep 'files changed'| awk -F ',' '{print $3}'|awk '{sum +=$1} END {print sum}'`
			if [[ ! $deleted ]];
				then 
				deleted=0
			fi
			
			#All lines for current user
			total=`expr $added + $deleted`
			
			printf "%-20s|%-20s|%-20s|%-20s\n" "${user}" "$added" "$deleted" "$total" >> "$report_file"
			printf "\n" >> "$report_file"
	done
